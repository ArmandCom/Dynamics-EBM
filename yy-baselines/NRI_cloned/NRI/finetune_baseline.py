...
parser.add_argument('--timesteps', type=int, default=69,
...
if args.save_folder:
    ...
    save_folder = '{}/exp{}/'.format(args.save_folder, 'edge_evaluation'+args.suffix)
...
class RecurrentBaseline(nn.Module):
    ...
    def forward(self, inputs, prediction_steps, burn_in=False, burn_in_steps=1):
        ...
        outputs = []
        hidden = None
        # print(inputs.shape, prediction_steps)
        # TODO: add nodes to batch at this level
        ...
        return outputs, hidden[0][-1].view(outputs.size(0), outputs.size(1), -1)
class LinearModel(nn.Module):
    """LSTM model for joint trajectory prediction."""

    def __init__(self, n_in):
        super(LinearModel, self).__init__()
        self.fc1_1 = nn.Linear(n_in, 1)

    def forward(self, inputs):
        output = self.fc1_1(inputs)
        return output

model = RecurrentBaseline(args.dims, args.hidden, args.dims,
                          args.num_atoms, args.num_layers, args.dropout)

linear_model = LinearModel(args.hidden * 2)


args.load_folder = '{}/exp{}/'.format(args.save_folder, 'edge_evaluation'+args.suffix)
model_file = os.path.join(args.load_folder, 'model.pt')
model.load_state_dict(torch.load(model_file))
args.save_folder = False

optimizer = optim.Adam(list(linear_model.parameters()), lr=args.lr)
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        data, relations = Variable(data), Variable(relations) # 64,20

        optimizer.zero_grad()

        with torch.no_grad():
            output, hidden = model(data, 100,
                           burn_in=True,
                           burn_in_steps=args.timesteps - args.prediction_steps)
        hidden = Variable(hidden)

        hidden_edges = node2edge(hidden, rel_rec, rel_send)

        pred_logits = linear_model(hidden_edges)
        ...
        loss = bceloss(pred_edge[..., 0], target.float())
        loss.backward()
        optimizer.step()
    ...
    print('Epoch: {:04d}'.format(epoch),
          'train acc: {:.10f}'.format(np.mean(train_accuracy)),
          'test acc: {:.10f}'.format(np.mean(test_accuracy))
          )
    # log.flush()
