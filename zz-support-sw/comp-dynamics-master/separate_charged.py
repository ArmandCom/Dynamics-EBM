...
if torch.cuda.is_available():
    train_loc_file = 'loc_train_charged%d.npy' %n
    train_vel_file = 'vel_train_charged%d.npy' %n
    train_edges_file = 'edges_train_charged%d.npy' %n
    test_loc_file = 'loc_test_charged%d.npy' %n
    test_vel_file = 'vel_test_charged%d.npy' %n
    test_edges_file = 'edges_test_charged%d.npy' %n
else:
    train_loc_file = '../../../Desktop/loc_train_charged%d.npy' %n
    train_vel_file = '../../../Desktop/vel_train_charged%d.npy' %n
    train_edges_file = '../../../Desktop/edges_train_charged%d.npy' %n
    test_loc_file = '../../../Desktop/loc_test_charged%d.npy' %n
    test_vel_file = '../../../Desktop/vel_test_charged%d.npy' %n
    test_edges_file = '../../../Desktop/edges_test_charged%d.npy' %n
...
def train(model, dataloader, initial_loc, initial_vel, initial_edges, sampling='MPPI', antidataloader=None, writer=None, name='Trajectory', epoch_num=0, \
          steps=200, std_dev=0.1, step_size=1, temperature=10, lr=10, inertia=False, dist=1, buffer=None, regularizer=True, backprop=True):
    if lr == '10':
    ...
    for epoch in range(100):
        for trajectory in dataloader:
            ...
            samples = sample_train(model, initial_loc, initial_vel, initial_edges, sampling, num_samples, writer=writer, name=name, epoch=epoch_num, \
                                   steps=steps, std_dev=std_dev, step_size=step_size, temperature=temperature, inertia=inertia, dist=dist, buffer=buffer)
...
def sample_train(model, initial_loc, initial_vel, initial_edges, sampling='MPPI', num_samples=256, length=None, writer=None, name='Trajectory', epoch=0, \
                 steps=200, std_dev=0.1, step_size=1, temperature=10, inertia=False, dist=1, buffer=None, display=False):
    if sampling.lower() == 'mppi':
        return sample_train_mppi(model, initial_loc, initial_vel, initial_edges, num_samples=num_samples, length=length, writer=writer, name=name, \
                                 epoch=epoch, steps=steps, std_dev=std_dev, temperature=temperature, inertia=inertia, dist=dist, buffer=buffer, display=display)
    if sampling.lower() == 'langevin':
        return sample_train_langevin(model, initial_loc, initial_vel, initial_edges, num_samples=num_samples, length=length, writer=writer, \
                                     name=name, epoch=epoch, steps=steps, std_dev=std_dev, step_size=step_size, inertia=inertia, buffer=buffer, display=display)

def sample_test(trajectory_model, same_model, different_model, initial_loc, initial_vel, initial_edges, n, sampling='MPPI', num_samples=256, length=None, writer=None, \
                name='Test', epoch=0, steps=200, std_dev=0.1, step_size=1, temperature=10, random=False, inertia=False, dist=1, position=0, display=False, show_graph=False):
    if sampling.lower() == 'mppi':
        return sample_test_mppi(trajectory_model, same_model, different_model, initial_loc, initial_vel, initial_edges, n, num_samples=num_samples, \
    ...
    if sampling.lower() == 'langevin':
        return sample_test_langevin(trajectory_model, same_model, different_model, initial_loc, initial_vel, initial_edges, n, num_samples=num_samples, \
...
def sample_train_mppi(model, initial_loc, initial_vel, initial_edges, num_samples=256, length=None, writer=None, name='Trajectory', epoch=0, \
                      steps=200, std_dev=0.1, temperature=10, inertia=False, dist=1, buffer=None, display=False):
    with torch.no_grad():
...
def sample_test_mppi(trajectory_model, same_model, different_model, initial_loc, initial_vel, initial_edges, n, num_samples=256, length=None, writer=None, \
                     name='Test', epoch=0, steps=200, std_dev=0.1, temperature=10, random=False, inertia=False, dist=1, position=0, display=False, show_graph=False):
    with torch.no_grad():
        ...
        for ind in range(model_length, length):
            ...
            for i in range(steps):
                ...
                energy = torch.sum(trajectory_model(samples.reshape(-1, 4, model_length)).view(num_samples, num_random, n, 1), 2) + \
                         torch.sum(torch.clamp(edges, 0, 1).unsqueeze(1).expand(-1, num_random, -1, -1) * \
                                   same_model(torch.cat([torch.cat((samples[:,:,4*c1[j]:4*c1[j]+4,:], samples[:,:,4*c2[j]:4*c2[j]+4,:]), 2) \
                                   for j in range(n*n)], 2).view(-1, 8, model_length)).view(num_samples, num_random, n, n), (2, 3)).unsqueeze(2) + \
                         torch.sum(torch.clamp(torch.neg(edges), 0, 1).unsqueeze(1).expand(-1, num_random, -1, -1) * \
                                   different_model(torch.cat([torch.cat((samples[:,:,4*c1[j]:4*c1[j]+4,:], samples[:,:,4*c2[j]:4*c2[j]+4,:]), 2) \
                                   for j in range(n*n)], 2).view(-1, 8, model_length)).view(num_samples, num_random, n, n), (2, 3)).unsqueeze(2)
                ...
                if display or (writer and length != 15):
                    e = torch.sum(trajectory_model(traj[:1,:,:,ind-model_length+1:ind+1].reshape(-1, 4, model_length))) + \
                        torch.sum(torch.clamp(edges[0], 0, 1) * same_model(torch.cat([torch.cat((traj[:1,:,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], \
                                  traj[:1,:,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 2) for j in range(n*n)], 2).view(-1, 8, model_length)).view(n, n)) + \
                        torch.sum(torch.clamp(torch.neg(edges[0]), 0, 1) * different_model(torch.cat([torch.cat((traj[:1,:,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], \
                                  traj[:1,:,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 2) for j in range(n*n)], 2).view(-1, 8, model_length)).view(n, n))
        ...
        if writer:
            for i in range(min(4, len(traj))):
                ...
                s_energy = sum([torch.sum(torch.clamp(edges[i], 0, 1) * same_model(torch.cat([torch.cat((traj[i:i+1,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], \
                ...
                d_energy = sum([torch.sum(torch.clamp(torch.neg(edges[i]), 0, 1) * different_model(torch.cat([torch.cat((traj[i:i+1,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], \
                                traj[i:i+1,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 1) for j in range(n*n)], 1).view(-1, 8, model_length)).view(n, n)) \
                                for ind in range(model_length, length)]) / (torch.sum(edges[i]) * (length - model_length))
                if length == 15:
                    ...
                    border = 5
                writer.add_figure(tag, graph(traj[i].cpu(), border=border, t_energy=t_energy, s_energy=s_energy, d_energy=d_energy, training=True), epoch)
...
def sample_train_langevin(model, initial_loc, initial_vel, initial_edges, num_samples=256, length=None, writer=None, name='Trajectory', epoch=0, \
                          steps=200, std_dev=0.01, step_size=1, inertia=False, buffer=None, display=False):
    model_length = model.length
    num_channels = model.channels
...
def sample_test_langevin(trajectory_model, same_model, different_model, initial_loc, initial_vel, initial_edges, n, num_samples=256, length=None, writer=None, \
    ...
    for ind in range(model_length, length):
        ...
        for i in range(steps):
            ...
            energy = torch.sum(trajectory_model(traj[:,:,ind-model_length+1:ind+1].reshape(-1, 4, model_length)).view(num_samples, n, 1), 1) + \
                     torch.sum(torch.clamp(edges, 0, 1) * same_model(torch.cat([torch.cat((traj[:,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], traj[:,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 1) \
                               for j in range(n*n)], 1).view(-1, 8, model_length)).view(num_samples, n, n), (1, 2)).unsqueeze(1) + \
                     torch.sum(torch.clamp(torch.neg(edges), 0, 1) * different_model(torch.cat([torch.cat((traj[:,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], traj[:,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 1) \
                               for j in range(n*n)], 1).view(-1, 8, model_length)).view(num_samples, n, n), (1, 2)).unsqueeze(1)
            traj_grad = torch.autograd.grad([energy.sum()], [traj])[0]
            ...
            if display or (writer and length != 15):
                e = torch.sum(trajectory_model(traj[:1,:,ind-model_length+1:ind+1].reshape(-1, 4, model_length)).view(1, n, 1), 1) + \
                    torch.sum(torch.clamp(edges[:1], 0, 1) * same_model(torch.cat([torch.cat((traj[:1,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], traj[:1,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 1) \
                              for j in range(n*n)], 1).view(-1, 8, model_length)).view(1, n, n), (1, 2)).unsqueeze(1) + \
                    torch.sum(torch.clamp(torch.neg(edges[:1]), 0, 1) * different_model(torch.cat([torch.cat((traj[:1,4*c1[j]:4*c1[j]+4,ind-model_length+1:ind+1], traj[:1,4*c2[j]:4*c2[j]+4,ind-model_length+1:ind+1]), 1) \
                              for j in range(n*n)], 1).view(-1, 8, model_length)).view(1, n, n), (1, 2)).unsqueeze(1)
    ...
    if writer:
        for i in range(min(4, len(traj))):
            ...
            if length == 15:
                ...
                tag = '%s_Sample_%d' %(string, i+1)
                border = 5
            writer.add_figure(tag, graph(traj[i].detach().cpu(), border=border, t_energy=t_energy, s_energy=s_energy, d_energy=d_energy, training=True), epoch)
...
def inference(same_model, different_model, traj, n, display=False, cutoff='mean'):
    with torch.no_grad():
        model_length = same_model.length
        num_traj = len(traj)
        length = len(traj[0][0])
        ...
        for ind in range(model_length, length):
            edges = edges - same_model(torch.cat([torch.cat((traj[:,4*c1[i]:4*c1[i]+4,ind-model_length+1:ind+1], traj[:,4*c2[i]:4*c2[i]+4,ind-model_length+1:ind+1]), 1) \
                                                  for i in range(n*n)], 1).view(-1, 8, model_length)).view(num_traj, n, n) + \
                            different_model(torch.cat([torch.cat((traj[:,4*c1[i]:4*c1[i]+4,ind-model_length+1:ind+1], traj[:,4*c2[i]:4*c2[i]+4,ind-model_length+1:ind+1]), 1) \
                                                       for i in range(n*n)], 1).view(-1, 8, model_length)).view(num_traj, n, n)
        ...
        edges = torch.sign(edges - boundary)
        for i in range(n):
            edges[:,i,i] = 0
        return edges

def test_prediction(trajectory_model, same_model, different_model, test_loc, test_vel, test_edges, n, sampling='MPPI', writer=None, name='Test', epoch=0, \
    ...
    for inertia in [True, False]:
        for position in [0, 20, 40, 50, 60, 70, 80]:
            traj, ind = sample_test(trajectory_model, same_model, different_model, test_loc, test_vel, test_edges, n, sampling=sampling, writer=writer, \
                                    name=name, epoch=epoch, num_samples=num_samples, length=length, steps=steps, std_dev=std_dev, step_size=step_size, \
...
def test_inference(same_model, different_model, test_loc, test_vel, test_edges, n, num_to_test=1000, cutoff='Mean', writer=None, name='Test', epoch=0, training=False):  
    ...
    guess_edges = inference(same_model, different_model, traj, n, cutoff=cutoff)

    actual = torch.sum(torch.clamp(test_edges, 0, 1)).item() / num_to_test
    guess = torch.sum(torch.clamp(guess_edges, 0, 1)).item() / num_to_test
    pos = torch.sum(torch.clamp(torch.clamp(guess_edges, 0, 1) - torch.clamp(test_edges, 0, 1), 0, 1)).item() / num_to_test
    neg = torch.sum(torch.clamp(torch.clamp(torch.neg(guess_edges), 0, 1) - torch.clamp(torch.neg(test_edges), 0, 1), 0, 1)).item() / num_to_test
    ...
    if not training or not writer:
        ...
        print('%s Extra 1s: %.3f' %(name, pos))
        print('%s Extra -1s: %.3f' %(name, neg))
        print('%s Total incorrect: %.3f' %(name, pos+neg))
    else:
        if type(cutoff) == str:
            writer.add_scalar('Num_Guessed Interactions_%s_%s' %(cutoff, name), guess, epoch)
            writer.add_scalar('Extra_Same_Guessed_%s_%s' %(cutoff, name), pos, epoch)
            writer.add_scalar('Extra_Different_Guessed_%s_%s' %(cutoff, name), neg, epoch)
            writer.add_scalar('Num_Incorrect_%s_%s' %(cutoff, name), pos + neg, epoch)
        elif type(cutoff) == int:
            writer.add_scalar('Num_Guessed Interactions_%d_%s' %(cutoff, name), guess, epoch)
            writer.add_scalar('Extra_Same_Guessed_%d_%s' %(cutoff, name), pos, epoch)
            writer.add_scalar('Extra_Different_Guessed_%d_%s' %(cutoff, name), neg, epoch)
            writer.add_scalar('Num_Incorrect_%d_%s' %(cutoff, name), pos + neg, epoch)
        else:
            writer.add_scalar('Num_Guessed Interactions_%.1f_%s' %(cutoff, name), guess, epoch)
            writer.add_scalar('Extra_Same_Guessed_%.1f_%s' %(cutoff, name), pos, epoch)
            writer.add_scalar('Extra_Different_Guessed_%.1f_%s' %(cutoff, name), neg, epoch)
            writer.add_scalar('Num_Incorrect_%.1f_%s' %(cutoff, name), pos + neg, epoch)
        if epoch == 0:
            print('Num Actual Same: %.3f' %actual)
...
def graph(traj, n=5, length=None, border=5, t_energy=None, s_energy=None, d_energy=None, graph_vel=False, includes_vel=True, transformed=True, training=False, ground_truth=False, save=False):
    fig, ax = plt.subplots()
    ...
    if t_energy or s_energy or d_energy:
        title = title + ' ('
        if t_energy:
            title = title + 'T: %.4f' %t_energy + ('; ' if s_energy or d_energy else '')
        if s_energy:
            title = title + 'S: %.4f' %s_energy + ('; ' if d_energy else '')
        if d_energy:
            title = title + 'D: %.4f' %d_energy
        title = title + ')'
...
def main():
    ...
    same_model = Interaction_Model(length, convolute=False)
    different_model = Interaction_Model(length, convolute=False)
