...
PATH1 = sub_folder + 'springs_trajectory_net.pth'
PATH2 = sub_folder + 'springs_interaction_net.pth'
output_file = sub_folder + 'springs_trajectory.npy'
...
def main():
    ...
    interaction_model = Interaction_Model(length, units=units, activation=activation, convolute=convolute)
    interaction_model.cuda()
    if dist != 1:
    ...
    interaction_dataset = Interaction_Data(train_loc, train_vel, train_edges, n=n, length=length, dist=dist)
    interaction_dataloader = DataLoader(interaction_dataset, batch_size=256, shuffle=True, drop_last=True)
    if anti:
        antiinteraction_dataset = Interaction_Data(train_loc, train_vel, train_edges, n=n, length=length, dist=dist, interaction=0)
        antiinteraction_dataloader = DataLoader(antiinteraction_dataset, batch_size=128, shuffle=True, drop_last=True)
    else:
        antiinteraction_dataloader = None
    if start > 1:
        trajectory_model.load_state_dict(torch.load(PATH1))
        interaction_model.load_state_dict(torch.load(PATH2))
    ...
    if start == 1:
        ...
        print('.')
        test_prediction(trajectory_model, interaction_model, test_loc, test_vel, test_edges, n, sampling=sampling, writer=writer, steps=steps, std_dev=std_dev, step_size=step_size, temperature=temperature, epoch=0, dist=dist)
        for c in ['Mean', 'Median', 0]:
            print('.')
            test_inference(interaction_model, train_loc, train_vel, train_edges, n, num_to_test=num_train, cutoff=c, writer=writer, name='Train', epoch=0, training=True)
            test_inference(interaction_model, test_loc, test_vel, test_edges, n, num_to_test=1000, cutoff=c, writer=writer, name='Test', epoch=0, training=True)
    ...
    for i in range(start, end):
        ...
        print('-')
        train(interaction_model, interaction_dataloader, test_loc, test_vel, test_edges, sampling=sampling, antidataloader=antiinteraction_dataloader, writer=writer, name='Interaction', epoch_num=i, steps=steps, std_dev=std_dev, step_size=step_size, temperature=temperature, inertia=inertia, dist=dist, buffer=buf2, regularizer=True, lr=lr, backprop=args.backprop)
        torch.save(interaction_model.state_dict(), PATH2)
        torch.save(interaction_model.state_dict(), sub_folder + 'interaction_%d.pth' %i)

        print('.')
        test_prediction(trajectory_model, interaction_model, test_loc, test_vel, test_edges, n, sampling=sampling, writer=writer, steps=steps, std_dev=std_dev, step_size=step_size, temperature=temperature, epoch=i, dist=dist)
        print('.')
        for c in ['Mean', 'Median', 0]:
            test_inference(interaction_model, train_loc, train_vel, train_edges, n, num_to_test=num_train, cutoff=c, writer=writer, name='Train', epoch=i, training=True)
            test_inference(interaction_model, test_loc, test_vel, test_edges, n, num_to_test=1000, cutoff=c, writer=writer, name='Test', epoch=i, training=True)

        print('-')
        for inert in [True, False]:
            if dist == 1:
                sample_test(trajectory_model, interaction_model, test_loc, test_vel, test_edges, n, sampling, 4, 100, writer=writer, epoch=i, steps=40, std_dev=std_dev, step_size=step_size, temperature=temperature, inertia=inert, dist=dist).cpu()
            else:
