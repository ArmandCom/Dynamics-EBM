...
from third_party.utils import save_rel_matrices, smooth_trajectory, get_model_grad_norm, get_model_grad_max
...
def train(train_dataloader, test_dataloader, logger, models, models_ema, optimizers, FLAGS, logdir, rank_idx, replay_buffer=None):
    ...
    schedulers = [CosineAnnealingLR(optimizer, T_max=150000, eta_min=2e-5, last_epoch=-1) for optimizer in optimizers]
    ...
    for epoch in range(FLAGS.num_epoch):
        for (feat, edges), (rel_rec, rel_send), idx in train_dataloader:
            ...
            loss = loss + feat_loss #+ 0.01 * torch.pow(energy_neg, 2).mean()#+ feat_loss_l1
...
def main():
    ...
    FLAGS.sample = True
    FLAGS.exp = FLAGS.dataset + FLAGS.exp
