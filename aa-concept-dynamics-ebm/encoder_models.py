...
class CNNMultipleLatentEncoder(nn.Module):
	def __init__(self, n_in, n_hid, n_out, n_latents=2, do_prob=0., factor=True):
...
# class OSMultipleLatentEncoder(nn.Module):
# 	def __init__(self, n_in, n_hid, n_out, n_latents=2, do_prob=0., factor=True):
# 		super(OSMultipleLatentEncoder, self).__init__()
# 		self.dropout_prob = do_prob
#
# 		self.factor = factor
# 		self.n_latents = n_latents
#
# 		self.cnn = MLP(n_in * 2, n_hid // 2, n_hid, do_prob)
# 		self.mlp1 = MLP(n_hid, n_hid, n_hid, do_prob)
# 		self.mlp2 = MLP(n_hid, n_hid, n_hid, do_prob)
# 		self.mlp3 = MLP(n_hid * 3, n_hid, n_hid, do_prob)
# 		self.fc_out = nn.Linear(n_hid, n_latents * n_out)
...
# 		out = self.fc_out(x)
#
# 		# outs = torch.split(out, self.n_latents, dim=-1)
