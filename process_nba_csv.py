...
for file_id in os.listdir(csv_dir):
	...
	for game_no in data_by_game.groups.keys():
		...
		for event_no in data_by_event.groups.keys():
			...
			try: assert all(df_i.iloc[::11]['team_id'] == -1)
			except: print('Divisible by 11, but first player is not -1'); continue
			if by_event:
				all_seconds.append(df_i)


			elif len(all_seconds) > 0:
	...
	for sec in all_seconds:
		...
		for player_id in players_group.groups.keys():
			print(player_id, players_group.get_group(player_id)[['x_loc', 'y_loc','game_clock']])
			try:
	...
	final_numpy_list = [np.flip(elem,1) for elem in final_numpy_list if elem.shape[1]>1]
